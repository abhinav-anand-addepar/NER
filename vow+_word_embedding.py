...
maxlen  =  0 
def addvowel( fulldataset1 ):
    fulldataset = fulldataset1 
    mm=0 
    for i, dataset in enumerate(fulldataset):
        for j, data in enumerate(dataset):
            ...
            fulldataset[i][j] = [data[0],  count , data[1]]
    return  fulldataset  , mm


trainv, maxlen1= addvowel(train) 
testv , maxlen2 = addvowel(test) 
validv , maxlen3    = addvowel(valid ) 
...
for data in [trainv,testv,validv]:
    for sentence in data:
        for word, vowcount ,  token in sentence:
            labels.add(token)
...
def creatematrix(data,wordid , labelid):
    ...
    for sentence in data:
        ...
        for word, vowcount, label in sentence:
        ...
        new_data.append([wordidxfinal, aa , labelidxfinal ])
...
trainf = creatematrix(trainv , word2Idx , label2Idx   )
testf = creatematrix(testv  , word2Idx , label2Idx   )
validf = creatematrix(validv   , word2Idx , label2Idx   )
...
def iterate_minibatches(dataset, batch_len):
    ...
    for i in batch_len:
        ...
        yield np.asarray(labels), np.asarray(voww ) ,  np.asarray(tokens) 
...
vowEmbeddings = np.identity( (maxlen + 1 ), dtype='float32')


words_input = Input(shape=(None,),dtype='int32',name='words_input')
words = Embedding(input_dim=wordEmbeddings.shape[0], output_dim=wordEmbeddings.shape[1],  weights=[wordEmbeddings], trainable=False)(words_input)
vow_input = Input(shape=(None,), dtype='int32', name='vow_input')
vow = Embedding(output_dim=10, input_dim=10, weights=[vowEmbeddings], trainable=False)(vow_input)

output = concatenate([words, vow ])
output = Bidirectional(LSTM(200, return_sequences=True, dropout=0.50, recurrent_dropout=0.25))( output )
output = TimeDistributed(Dense(len(label2Idx), activation='softmax'))(output)
model = Model(inputs=[words_input  , vow_input], outputs=[output])
...
def tag_dataset(dataset):
    ...
    for i,data in enumerate(dataset):    
        tokens,  vow ,              labels = data
        tokens = np.asarray([tokens])     
        
        vow  = np.asarray([ vow ])
        pred = model.predict([tokens , vow  ], verbose=False)[0]   
...
for epoch in range(epochs):    
    ...
    for i,batch in enumerate(iterate_minibatches(train_batch,train_batch_len)):
        labels,vow,  tokens  = batch       
        model.train_on_batch([tokens  , vow ], labels)
        a.update(i)
